...
// 解析 HEX 字串為陣列
vector<string> parseHexFile(ifstream& ins, string maxLines) {
    vector<string> hexData;
    string word;
    int iwords = 0;
    int ImaxLines = stoi(maxLines);
    while (ins >> word) {
      hexData.push_back(word);
      if(iwords>ImaxLines){
        if(iwords >= 3&&isSyncEnd(hexData,iwords)){
          break;
        }
      }
      iwords++;
    }
    cout<<"L79:  , hex array size: "<<hexData.size()<<endl;
    return hexData;
}
// 解析封包
vector<Packet> extractPackets(const vector<string>& hexData , int steps = 0) {
  ...
  if(steps == -1){
    packets[packets.size()-1].isValid = false;
    packets[packets.size()-1].bSync = false;
    packets[packets.size()-1].invalidReason = "Tail of File";
    cout<<" iStep = "<<steps<<endl;
  }
  ...
  for (size_t j = 0; j < packets.size() - 1; j++) {
    if(!packets[j].bSync){
      if(packets[j + 1].data.size()%3!=0){
        ...
        if(j!=packets.size() - 2) packets[j + 1].isValid = true;
      }
    }
  }
...
void DecodingV2(
 ...
 const int iDetector = 0
  ...
  ifstream infile(HexRawSciFileName);
  double Eff = 0, Tot = 0, Loss = 0;
  int  steps = 0;
  stringstream ofiss;
  vector<Packet> packets0;
  while(true){
    vector<string> hexData = parseHexFile(infile,"10000000");
    if(hexData.size()==0) break;
    if(hexData.size()<10000000) steps = -1;   cout<<"L316 : steps: "<<steps<<endl;
    vector<Packet> packets = extractPackets(hexData, steps);
    cout<<"L318 : TotPakSize: "<<packets.size()<<" TotPakSize0: "<<packets0.size()<<endl;
    packets.insert(packets.begin(), packets0.begin(), packets0.end());
    cout<<"L320 : TotPakSize+0: "<<packets.size()<<endl;
    vector<Packet>().swap(packets0);
    cout<<"L322 : TotPakSize 0: "<<packets0.size()<<endl;
    size_t TotPakSize = packets.size();
    size_t UsedPakSize = 0;
    bool bOP = 1;
    for (const auto& pkt : packets){
      if(!bOP) break;
      UsedPakSize++;
      ...
      if(!pkt.isValid){
        ...
        if((TotPakSize-UsedPakSize)<=3&& steps!=-1){
          UsedPakSize--;
          Tot -= pkt.data.size();
          
          cout<<"L327 : TotPakSize - UsedPakSize: "<<(TotPakSize-UsedPakSize)<<endl;
          vector<Packet> remaining(packets.begin() + UsedPakSize, packets.end());
          cout<<"L329 : remaining: "<<remaining.size()<<endl;
          packets0.swap(remaining);
          cout<<"L331 : TotPakSize_0: "<<remaining.size()<<endl;
          break;
        }
        stringstream ssSync;
        for (const auto& word : pkt.data) {
        ...
        while(ssData>>Hitdata[0]>>Hitdata[1]>>Hitdata[2]){
          ...
          if     (((Word>>23)&1) == 0){// cout<<"<---Find Header \"0\" for Event-Hit data"<<endl;
            ...
            if((TotPakSize-UsedPakSize)<=3&& steps!=-1){
              ...
              while(ssData>>Hitdata[0]) dataTMP.push_back(Hitdata[0]);
              vector<Packet> PakHalfUsed={{dataTMP,pkt.isValid,pkt.bSync,pkt.invalidReason}};
              
              cout<<"L569 : TotPakSize - UsedPakSize: "<<(TotPakSize-UsedPakSize)<<endl;
              vector<Packet> remaining(packets.begin() + UsedPakSize, packets.end());
              cout<<"L571 : remaining: "<<remaining.size()<<endl;
              packets0.swap(remaining);
              cout<<"L573 : remaining: "<<remaining.size()<<endl;
              cout<<"L574 : packets0: "<<packets0.size()<<endl;
              packets0.insert(packets0.begin(), PakHalfUsed.begin(), PakHalfUsed.end());
              cout << "PakHalfUsed.size() = " << PakHalfUsed.size() << endl;
              cout << "L576 : packets0.size() after insert = " << packets0.size() << endl;
              bOP = false;
              break;
            }
          }
...
void DecodingV2(){
  cout<<"Finished compiling of DecodingV2.C+"<<endl;
}