...
// void DrawTriggerLinesMax(const string  RootFileName = defRFM,const string SavePath = defSP) {
void DrawTriggerLinesMax(const string  RootFileName ,const string SavePath ) {
    setTDRStyle();
    ...
    c3->cd()->Divide(2,2);
    TH2F *h2DiffB1 = new TH2F("h2DiffB1","Position Difference @LYSO;#Delta X (mm);#Delta Y position (mm);ADC",71,-74.55,+74.55,71,-74.55,+74.55);
    TH2F *h2DiffB2 = new TH2F("h2DiffB2","Position Difference @PbWO4;#Delta X (mm);#Delta Y position (mm);ADC",71,-74.55,+74.55,71,-74.55,+74.55);
    TH2F *h2Diff1 = new TH2F("h2Diff1","Position Difference @LYSO;#Delta X (mm);#Delta Y position (mm);ADC",71,-74.55,+74.55,71,-74.55,+74.55);
    TH2F *h2Diff2 = new TH2F("h2Diff2","Position Difference @PbWO4;#Delta X (mm);#Delta Y (mm);ADC",71,-74.55,+74.55,71,-74.55,+74.55);
    TH2F *h2Diff2C = new TH2F("h2Diff2C","Position Difference(Combination)@PbWO4;#Delta X (mm);#Delta Y (mm);ADC",71,-74.55,+74.55,71,-74.55,+74.55);
    TH1F *h1DiffR = new TH1F("h1DiffR","#DeltaR@PbWO4;#Delta=#sqrt{#DeltaX^{2}+#DeltaY^{2}} (mm);count",84,0,120);
    TH2F *h2Dir = new TH2F("h2Dir","Direction;dX/dZ;dY/dZ;count",71,-1,1,71,-1,1);
    ...
    // for (Long64_t i = 0; i < 10; i++) {
      ...
      for(int iH = 0; iH<nHits;iH++){
        ...
        // cout<<iH<<" "<<iz<<" "<<ihit<<" "<<px<<" "<<py<<" "<<pz<<" "<<Weight+1200<<endl;
        if(iz==1&&ihit==0){
          Trigger2PosX.push_back(Hit(px,0,pz,Weight+1200));
          // Trigger2PosX.end()->Show();
        }
        else if(iz==2&&ihit==0) Trigger2PosY.push_back(Hit(0,py,pz,Weight+1200));
        else if(iz==3&&ihit==0) Trigger1PosX.push_back(Hit(px,0,pz,Weight+1200));
        else if(iz==4&&ihit==0) Trigger1PosY.push_back(Hit(0,py,pz,Weight+1200));
        else if(iz==5){
      ...
      // cout<<"CT1: "<<CenterT1.x<<" "<<CenterT1.y<<" "<<CenterT1.z<<" "<<CenterT1.w<<endl;
      // cout<<"CT2: "<<CenterT2.x<<" "<<CenterT2.y<<" "<<CenterT2.z<<" "<<CenterT2.w<<endl;
      // cout<<"CD2: "<<CenterD2.x<<" "<<CenterD2.y<<" "<<CenterD2.z<<" "<<CenterD2.w<<endl;
      // throw;
      ...
      if(i<20){
        ...
        c3->Print(Form("%s/C_%05.0f_MaxCH.gif",SavePath.data(),i*1.));
      }
      h2D0->Reset();
      h2D1->Reset();
      h2D2->Reset();
    ...
    layoutD2.canvas->Print(Form("%s/RSD_PosBMPDandZDCD%d_MaxCH.gif",SavePath.data(),2));
    setTDRStyle();
    TCanvas *c0 = new TCanvas("c0","c0",640+4,640+28); c0->cd();
    h1DiffR->Draw("he");
    c0->Print(Form("%s/RSD_Pos_R_MaxCH.gif",SavePath.data()));
    ...
    c1->cd(2);
    hThetaPhi->Draw("colz");
    c1->Print(Form("%s/Direction_MaxCH.gif",SavePath.data()));
    // 關閉文件
}

void DrawTriggerLinesMax() {
  cout<< "Finish compile: DrawTriggerLinesMax"<<endl;
  // DrawTriggerLinesMax("/data8/ZDC/EMCal/BeamTest/Feb25Sort/Feb25PWOOnly/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4_ReCon.root","/data8/ZDC/EMCal/BeamTest/Feb25Sort/Feb25PWOOnly/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4/graph");
  DrawTriggerLinesMax("/data8/ZDC/EMCal/BeamTest/Feb25Sort/Feb25PWOOnly/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4_ReCon_MDID4.root","/data8/ZDC/EMCal/BeamTest/Feb25Sort/Feb25PWOOnly/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4/graph/");
