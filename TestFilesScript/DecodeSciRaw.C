...
stringstream GetLastSyncData(ifstream &in, int &Cosume,int &lost,int &TotalSync){
  ...
  // cout<<ss.str()<<endl;
  cout<<"searching fine sync data 62"<<endl;
  ss<<"NAN";
  return ss;
}
stringstream GetLastSyncDataCA(ifstream &in,string &Str1,string &Str2,string &Str3, int &Cosume,int &lost,int &TotalSync){
  ...
  if(buffer[0]=="CA"&&(buffer[buffer.size()-3]+buffer[buffer.size()-2]+buffer[buffer.size()-1])=="F2F5FA"){
...
void DecodeSciRaw(
 ...
 const int iDetector = 0
  ...
  if(strSyncData.str()!=string("NAN")){
    ...
    while(true){ // Get 3 byte data for #Word 0      
      if (SetSyncMode) {
        strSyncData>>Hitdata[0]>>Hitdata[1]>>Hitdata[2];
        ByteIndex+=3;
      }else{
        if(!(inRawSci>>Hitdata[0]>>Hitdata[1]>>Hitdata[2])) break;
        ByteIndex+=3;
        // cout<<Hitdata[0]<<" "<<Hitdata[1]<<" "<<Hitdata[2]<<endl;
      }
      Word = GetWordVal(Hitdata);// transform 3 bytes data to be 1 word value(int)
      if(Hitdata[0]=="CA"){
        ...
        if     (((Word>>23)&1) == 0){// cout<<"<---Find Header \"0\" for Event-Hit data"<<endl;
          ...
          cout<<"  Try: consume 2 bytes data to revise!"<<endl;
          ofiss<<"Error: Can't find Header!!! Problem header:  "<<ByteIndex<<"  "<<Hitdata[0].data()<<endl;
          ofiss<<"  Try: consume 2 bytes data to revise!"<<endl;
