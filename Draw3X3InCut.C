...
double xoff = 10.25, yoff = 0;
double Step = 2.5;
vector< vector<double> > offsetSet={{0,0},{10.25,0},{10.25,10.25}};
void Draw3X3InCut( TTree *t,int DID){
  ...
  TH3D *h3 = new TH3D("h3",";DiX(iX-iX_{max});DiY(iY-iY_{max});Edep[MeV]",15,-7,7,15,-7,7,30,0,960);
  ...
  TH3D *h3C = new TH3D("h3C",";DiX;DiY;Edep[MeV]",15,-7,7,15,-7,7,30,0,960);
  ...
  for (int i = 0; i < NScans; i++) {
    ...
    t->Draw("Edep:DiY:DiX>>h3C",RSDcdts.Data(),"box2z");
    h3C->GetZaxis()->SetTitleOffset(1.6);
    h3C->GetXaxis()->CenterTitle(1);
    h3C->GetYaxis()->CenterTitle(1);
    h3C->GetXaxis()->SetRangeUser(-3,3);
    h3C->GetYaxis()->SetRangeUser(-3,3);
    cout<<RSDcdts<<endl;
    ...
    g->Draw("psame");
    canvas->Print(Form("%sADCVS_DiXiY_Track_WIcutRin%02.0f.gif",dirGraph .data(),currentR));
    canvas55->cd();
  ...
  leg5x5->Draw();
  canvas55->Print(Form("%sE5x5_DiXiY_TrackCutScan.gif",dirGraph.data()));
  ...
  canvas3->Divide(3,3,0.0,0.0); // 8x8 排列
  ...
  for(int iX = -1; iX<=1;iX++){
    for(int iY = -1; iY<=1;iY++){
      for (int i = NScans - 1; i >= 0; i--){
        ...
        if(iX==0&&iY==0){
          cProj->cd();
          if(i==5) hProjZ->Draw("eh");
          else     hProjZ->Draw("ehsame");
        }
        int padIndex = (iX+2)+(1-iY)*3;
        // cout<<" "<<158<<" "<<iX<<" "<<iY<<" "<<padIndex<<endl;
        canvas3->cd(padIndex)->SetGrid(1,1);
        canvas3->cd(padIndex)->SetLogy(1);
        double CSFX = 
          canvas3->GetPad(padIndex)->GetAbsWNDC() / canvas3->GetPad(9)->GetAbsWNDC();
        double CSFY = 
          canvas3->GetPad(padIndex)->GetAbsHNDC() / canvas3->GetPad(9)->GetAbsHNDC();
  ...
  TString GraphName = Form("EnergySlices_DiXiY_5x5_D%d_X%03.2fY%03.2f", DID, xoff, yoff);
  ...
  cProj->Print(Form("%sEnergySlices_DiXiY_Overlap_D%d.gif", dirGraph.data(), DID));
  ...
  leg[0][0]->Draw();
  cProj->Print(Form("%sEnergySlices_DiXiY_Overlap_log_D%d.gif", dirGraph.data(), DID));
    CleanupHistogramsAndCanvases();
}
void Draw3X3InCut( const string dirAnaPath ,  const string FileName ){
  ...
  if (!t) {
      ...
      std::cerr << "  Draw3X3InCut.C("<<dirAnaPath<<","<<FileName<<")"<< std::endl;
      return;
  }else{
    cout << "  Draw3X3InCut.C("<<file->GetName()<< std::endl;
  }
...
void Draw3X3InCut() {
  cout<<"Finished compiling of Draw3X3InCut.C+"<<endl;
  Draw3X3InCut("/data8/ZDC/EMCal/PbWO4SiPM/AnaCode1/Save/EScanTypicalRuns/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4/", "Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4");
}
