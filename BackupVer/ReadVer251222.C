...
vector<int>     iDetArr   = {1,2,3,4};
vector<string> SymbolFile = {"D1","T1","T2","D2"};

  /*Example of files path name*/
//The case file is path/CaseString_hhmmss.bin
vector<string> SourceFileNameBin = {"unUseValShouldChange","","",""};
// Binary Raw data Ex: "path/CaseString_hhmmss.bin"
vector<string> SourceFileNameHex = {"unUseValShouldChange","","",""};
// Convert to be Hex Raw data Ex: "path/CaseString_hhmmss.hex"
vector<string> HexRawSciFileName = {"unUseValShouldChange","","",""};
// Save hex science data to be Hex Ex: "path/CaseString_hhmmss_Sci.hex"
vector<string> ReadableRawSciFileNameT = {"unUseValShouldChange","","",""};
// Save hex science data to be Readable .txt Ex: "path/CaseString_hhmmss_Sci.dat"
vector<string> ReadableRawSciFileNameR = {"unUseValShouldChange","","",""};
...
void ReadVer251222(
  ...
  vector<bool> FileCheck={
    ChechName(NameRootMD1), ChechName(NameRootT1), ChechName(NameRootT2), ChechName(NameRootMD2) 
  };
  vector<string> FileNames={ NameRootMD1, NameRootT1, NameRootT2, NameRootMD2  };
  ...
  // throw;
  /*
  vector<string> scripeNames = {"BinToHex","SelectScidata","DecodeSciRaw","ReConstruct","DrawADC"};
  if(false){
    string cmdline = "";
    for(size_t i=0; i <scripeNames.size();i++)
  ...
  if((process>>3)%2){
    ...
    string cmdline = "";
    for(int i=0; i <4;i++)
      if(FileCheck[i]) cmdline+=string(Form("root -l -b -q SelectScidata.C+\\(\\\"%s\\\",\\\"%s\\\"\\)&",SourceFileNameHex[i].data(),HexRawSciFileName[i].data()));
    cmdline +="wait";
    system(cmdline.data());
  }
  if((process>>2)%2){
    ...
    if( posRun!=-1 ){
      ...
      for(int i=0; i <4;i++){
        if(FileCheck[i]){
          system(Form("ln %s/%s_Sci.root %s/Run%06.0f_%s_Sci.root",NameRoot.data(),SymbolFile[i].data(),NameRoot.data(),runIndex*1.,SymbolFile[i].data()));
        }
      }
    }
 ...
 // throw; // test break line
  ...
  if((process>>0)%2){
    ...
    system(Form("(root -l -b -q DrawADCRec.C+\\(\\\"%s\\\",\\\"%s\\\",%d\\))&(wait)",dirAnaPath.data(),fileName.data(),4));
