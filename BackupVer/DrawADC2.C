...
void DrawADC(TTree *t, const int iDet) {
  ...
  map< int, TH1D* > h1chs;
  map<pair<int,int>,vector<int>> viXiYGIDs;
  for(auto [igid , iS ] : sensorMap){
    h1chs[igid] = new TH1D(Form("h1ChiX%diY%d_GID%d",iS.iX,iS.iY,iS.GID),"",120,-1000,11000);
    viXiYGIDs[{iS.iX,iS.iY}].push_back(igid);
  }
  TH2D *hGADCVSGCh;
...
// --- 修正後的 Entry Loop (處理向量迭代) ---
  ...
  if(bBMs) h2ByH3ReduceZ->SetBins( NumCY, 0, NumCY, NumCX, 0, NumCX);
  for (int xbin = 1; xbin <= NumCX; ++xbin) {
    for (int ybin = 1; ybin <= NumCY; ++ybin) {
      int rootY = NumCY - ybin + 1;
      int rootPadIndex = (rootY - 1) * NumCX + xbin;
      
      double CSFX = canvas->GetPad(rootPadIndex)->GetAbsWNDC() / canvas->GetPad(int(NumCX*NumCY)/2)->GetAbsWNDC();
      double CSFY = canvas->GetPad(rootPadIndex)->GetAbsHNDC() / canvas->GetPad(int(NumCX*NumCY)/2)->GetAbsHNDC();
      
      vector<TH1D *>hProjZs;
      vector<int> GIDs;
      if(bBMs)
        hProjZs.push_back(h3->ProjectionZ(TString::Format("projZ_%d_%d", xbin, ybin), xbin, xbin, ybin, ybin, "cutg"));
        
      
      for(auto id : viXiYGIDs[{xbin-1,ybin-1}]){
        hProjZs.push_back(h1chs[id]);
        // cout<<id<<endl;
        GIDs.push_back(id);
        // cout<<id<<endl;
      }
      int ihs = 0;
      for(auto hProjZ:hProjZs){
        ihs++;
        if (bBMs) hProjZ->SetTitle(Form("GID = %d;ADC;Counts #scale[0.8]{x10^{%d}}", xbin - 1 + 8 * (ybin - 1), Digit_3D));
        else hProjZ->SetTitle(Form(";kADC;Counts #scale[0.8]{x10^{%d}}", Digit_3D));
        if(!bBMs) hProjZ->Scale(1 / pow(10, Digit_3D));
        hProjZ->SetMarkerColor(ihs);
        hProjZ->SetLineColor(ihs);
        ...
        hProjZ->Draw(ihs==1 ? "eh" : "ehsame");
        // 建立 TLatex 物件
        TLatex *tex = new TLatex();
        tex->SetNDC();
        if((ybin == xbin&&xbin == 1)) tex->SetTextSize(0.09/CSFX );
        else                           tex->SetTextSize(0.09);
        tex->SetTextAlign(32);  tex->SetTextFont(42);  tex->SetTextColor(ihs);
        hProjZ->Draw(ihs==1 ? "eh" : "ehsame");
        if(!bBMs) tex->DrawLatex(0.98, 1.05-0.1*ihs, Form("iX = %d, iY = %d, GID = %d", xbin - 1, ybin - 1, GIDs[ihs-1]));
        double mean = hProjZ->GetMean(), rms = hProjZ->GetRMS();
        TH1D *hProjZRange = (TH1D*)hProjZ->Clone((string(hProjZ->GetName())+("R")).data());
        ...
        tex2->SetNDC(); tex2->SetTextSize(0.09);
        tex2->SetTextAlign(22);  tex2->SetTextFont(42);  tex2->SetTextColor(ihs);
        hProjZRange->Draw(ihs==1 ? "eh" : "ehsame");
        if(!bBMs) tex2->DrawLatex(0.55, 1.05-0.1*ihs, Form("iX = %d, iY = %d, GID = %d", xbin - 1, ybin - 1, GIDs[ihs-1]));
        if(bBMs) h2ByH3ReduceZ->SetBinContent(ybin, xbin, hProjZRange->GetMean());
        else     h2ByH3ReduceZ->SetBinContent(xbin, ybin, hProjZRange->GetMean());
      }
    }
  }
...
void DrawADC() {
  ...
  // DrawADC("/data8/ZDC/EMCal/PbWO4SiPM/AnaCode1/Save/EScanTypicalRuns/Run2013_796MeV_HV17_VF650_650_x4_Pos145mm_-346mm_0mm_232554.122PbWO4/");
}
